...
def save_final_model(net, optimizer, epoch,save_path, distributed):
    if is_main_process():
        model_state_dict = net.state_dict()
        state = {'model': model_state_dict, 'optimizer': optimizer.state_dict()}
        # state = {'model': model_state_dict}
...
def get_work_dir(cfg):
    ...
    hyper_param_str = '_lr_%1.0e_ep_%d' % (cfg.learning_rate, cfg.epoch)
    work_dir = os.path.join(cfg.log_path, cfg.note + '_' + now + hyper_param_str)
    return work_dir
